...
void CreateTimeDatabase(uint32_t nEnc, char* realtimeFilename, char* plaintextFilename)
{
	...
	for (iEnc=0;iEnc<nEnc;iEnc++){
		...
		for(iRep = 0; iRep < nRep; iRep++) {
			a = rdtsc();
			ciphertext = DES_Encrypt(plaintext, ks);
			b = rdtsc();
			avg += (double)(b-a);
		}
...
int main(int argc, char **argv) {
	...
	sprintf( realtimeFilename, "data/2/des_time.dat");
	sprintf( plaintextFilename, "data/2/des_plaintext.dat");
	printf("%s\n", plaintextFilename);
	//CreateTimeDatabase(nEnc[9], realtimeFilename, plaintextFilename);

	char resultFilename2[40];
	/*
	char resultFilename1[40];
	char resultFilename2[40];
	sprintf( resultFilename, "data/pcc2sbox%d_top3_6291456.dat", 1);
	printf("%s\n", resultFilename);
	TimingAttackDES_TwoSbox(nEnc[2] + nEnc[1], realtimeFilename, plaintextFilename, 					resultFilename, 0);
